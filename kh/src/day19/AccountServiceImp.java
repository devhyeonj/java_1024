...
import example.accountbook.copy.AccountService;
import example.accountbook.copy.Item;
...
public class AccountServiceImp implements AccountService{
	...
	public void runPrint(ArrayList<Item> list,int subMenu, Scanner scan) {
		...
		case 2:
			year = inputYear(scan);
			//printItem(list, year);
			//printItem(list, (tmp)-> tmp.getDate().getYear() == year);
			printItem(list,(s) -> s.getDateStr().substring(0, 4).equals(""+year));
			break;
		case 3:
			year = inputYear(scan);
			month = inputMonth(scan);
			//printItem(list, (tmp)-> tmp.getDate().getYear() == year && tmp.getDate().getMonth() == month);
		...
		case 4:
			...
			month = inputMonth(scan);
			day = inputDay(scan);
			//printItem(list, (tmp)-> tmp.getDate().getYear() == year&& tmp.getDate().getMonth() == month && tmp.getDate().getDay() == day);
