...
public class BoardMain {
	...
	/*
	 * 게시글 관리 프로그램 - 회원가입 - 처음 가입한 회원이 관리자, 그외 회원인 일반 회원 - 로그인 - 게시글 등록/수정/삭제 => 회원만
	 * 가능 - 카테고리 관리(공지, 자유) - 카테고리 추가/수정/삭제(관리자만 가능)
	 * 
	 * -기능 - 회원관리 - 로그인 - 회원가입 - 게시글 관리 - 게시글 등록 (회원만) - 게시글 수정 (회원 + 작성자 본인) - 게시글
	 * 삭제 (회원 + 작성자 본인) - 게시글 목록 - 게시글 검색(검색 번호는 게시글 번호로) - 게시글 확인 - 카테고리 관리(회원 +
	 * 관리자만) - 카테고리 추가 - 카테고리 수정 - 카테고리 삭제
	 */
	...
	private static void printBoardSearch() {
		...
		printBoardList(b -> b.getTitle().contains(search.trim())); // 앞뒤공백 제거 그러면 검색어를 입력안하고 공백을 입력하면 전체게시글이 나옴
	}
	...
	private static void updateBoard() {
		// 회원 체크 => 회원(로그인한 사용자)이 아니면 게시글 수정 못함
		if (checkLogin(false)) {
			return;
		}
		// 수정할 게시글 번호 입력
	...
	private static void insertBoard() {
		...
		} while (categoryList.contains(category)); // 카테고리 리스트에 내가 입력한게 없으면 반복
		System.out.print("제목 : ");
		String title = scan.nextLine();
		System.out.print("내용 : ");
		String contents = scan.nextLine();
