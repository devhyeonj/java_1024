...
<div class="container">
	...
	<table class="table table-hover">
	 ...
	 <c:url value='/board/list?page=${i}'></c:url>
    ...
    <c:if test="${pm.prev}">
	 ...
	 <c:forEach begin="${pm.startPage}" end="${pm.endPage}" var="i">
	    ...
	    <li class="page-item <c:if test="${pm.cri.page == i}"> active</c:if>"><a class="page-link" href="#">${i}</a></li>
	 	
	 </c:forEach>
	 <c:if test="${pm.next}">
	    <li class="page-item"><a class="page-link" href="<c:url value='/board/list?page=${pm.endPage+1}'></c:url>0">다음</a></li>
     </c:if>
    </ul>
