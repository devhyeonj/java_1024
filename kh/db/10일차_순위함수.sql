...
*/
select 
	row_number() over(order by pr_amount desc) 보유량순위 ,
	pr_title as 제품명,
    pr_amount as 보유량
from product;

select 
	dense_rank() over(order by pr_price desc) 가격순위 ,
	pr_title 제품명,
    pr_price '제품 가격'
from product;
...
select 
	rank() over(order by pr_price desc) 가격순위 ,
	pr_title 제품명,
    pr_price '제품 가격'
from product;

select 
	ntile(3) over(order by pr_price desc) 가격순위 ,
	pr_title 제품명,
    pr_price '제품 가격'
from product
...
select *, 
	lead(pr_title) over (ORDER BY pr_num asc) 다음행값 from product;

select 
	row_number() over(order by pr_price desc)
    pr_title,pr_price,
    pr_price - lead(pr_price,1,pr_price) over(order by pr_price desc) as 가격차이 from product;
    
select 
	row_number() over(order by pr_price desc)
    pr_title,pr_price,
    pr_price - lag(pr_price,1,pr_price) over(order by pr_price desc) as 가격차이 from product;
...
select pr_pc_num 분류번호 , avg(pr_price) 평균가격 
	from product
	group by(pr_pc_num);
